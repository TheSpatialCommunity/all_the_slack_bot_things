...
@app.route('/simplenewuser/<email>')
def simple_new_user(email):

    token = CONFIG.get('tokens', "SIMPLE_NEWUSER_TOKEN")
    invite_user_payload = json.dumps({
        "token": token,
        "channel": "G32PUBP1R",
	    "username":"superinvite",
	    "text": "{} would like an invite to The Spatial Community. If you approve, add an emoji (a :heavy_check_mark: will do) indicating you're sending the invite, then copy the email and invite the user from the Slack workspace menu.".format(email),
        "attachments": [{"title":"A New Player Has Entered The Game.",}]
    })

    headers = {"Content-Type": "application/json", "Authorization": "Bearer {}".format(token)}
    response = requests.post(CONFIG.get("webhooks", "SIMPLE_NEWUSER_WEBHOOK_URL"), headers=headers, data=invite_user_payload)
...
def submit_email(email):
    ...
    parameters = {'token':bot_token, 'text':email,'attachments': invite_user_attach, 'channel':channel,
                'username':bot_username,'as_user':'true'}
    response = requests.get("https://slack.com/api/chat.postMessage",params=parameters)
    return jsonify({"status":response.status_code, "response": response.json()})
...
def button_response():
    ...
    if action_ == 'invite':
        ...
        f = {'token': bot_token, 'email': requestor}
        x = requests.post('https://slack.com/api/users.admin.invite?' + urllib.parse.urlencode(f))
    ...
    return ("{user} has {action} {email}").format(user=button_presser,action=action_taken,email=requestor)
...
def markov(**kwargs):
    ...
    # return slack.response(str(channels_response), response_type='emphemeral')
